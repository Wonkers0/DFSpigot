		...
		switch (action) {
			...
			case "BreakBlock": {
				...
				for (Entity ps : loc.getWorld().getNearbyEntities(loc,64,64,64))
					if (ps instanceof Player) ((CraftPlayer) target).getHandle().connection.send(new ClientboundLevelEventPacket(2001, new BlockPos(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()), Block.getId(((CraftBlockData) loc.getBlock().getBlockData()).getState()), false));
