...
public class PlayerAction extends Action {
			...
			switch(action){
				...
				case "GiveExp": {
					...
					switch (tags.get("Give Experience")) {
						case "Points" -> player.giveExp(args.get("amount").getInt());
						case "Level" -> player.giveExpLevels(args.get("amount").getInt());
				...
				case "SetExp": {
					...
					switch (tags.get("Set Experience")) {
						case "Points" -> player.setTotalExperience(args.get("amount").getInt());
						case "Level" -> player.setLevel(args.get("amount").getInt());
						case "Level Percentage" -> player.setExp(Math.min((float) (double) args.get("amount").getVal() / 100, 1));
					}
					break;
				}
				...
				case "DisplayHologram": {
					...
					Hologram.showHologram(loc, text, (Player) target);
				...
				case "SetChatTag": {
					PlayerData playerData = PlayerData.getPlayerData(target.getUniqueId());
					playerData.chatTag = String.join("", DFValue.castTxt((DFValue[]) args.get("tag").getVal()));
					break;
				}
	...
	private static void drawParticleCircle(Player player, DFParticle particle, Location loc, double diameter, long ticks){
		// TODO: Does not animate correctly due to different implementation
//		Vector dir = loc.getDirection().normalize();
//		double wait = 0;
//
//		for(int n = 1; n <= 360; n++) {
