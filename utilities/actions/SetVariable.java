	...
	public void invokeAction(){
		...
		for(Entity ignored : DFUtilities.getTargets(targetName, targetMap, SelectionType.EITHER)) {
			...
			switch (action) {
				...
				case "GetParticleType" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = (DFParticle) args.get("particle").getVal();
					DFVar.setVar(var, new DFValue(particle.name, DFType.TXT), localStorage);
				}
				
				case "SetParticleType" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = args.get("particle") == null ? (DFParticle) DFVar.getVar(var, localStorage).getVal() : (DFParticle) args.get("particle").getVal();
					particle.name = (String) args.get("type").getVal();
					
					DFVar.setVar(var, new DFValue(particle, DFType.PART), localStorage);
				}
				
				case "GetParticleAmount" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = (DFParticle) args.get("particle").getVal();
					
					DFVar.setVar(var, new DFValue(particle.amount, DFType.NUM), localStorage);
				}
				
				case "SetParticleAmount" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = args.get("particle") == null ? (DFParticle) DFVar.getVar(var, localStorage).getVal() : (DFParticle) args.get("particle").getVal();
					particle.amount = args.get("amount").getInt();
					
					DFVar.setVar(var, new DFValue(particle, DFType.PART), localStorage);
				}
				
				case "GetParticleSprd" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = (DFParticle) args.get("particle").getVal(); // Keep in mind that DF Does not allow this as a potential argument
																																					  // (for this particular action. For others it does, it's weird)
					
					DFVar.setVar(var, new DFValue(tags.get("Spread").equals("Horizontal") ? particle.hSpread : particle.vSpread, DFType.NUM), localStorage);
				}
				
				case "SetParticleSprd" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = args.get("particle") == null ? (DFParticle) DFVar.getVar(var, localStorage).getVal() : (DFParticle) args.get("particle").getVal();
					Double hSpread = (Double) args.get("horizontal").getVal();
					Double vSpread = (Double) args.get("vertical").getVal();
					
					if(hSpread != null) particle.hSpread = hSpread;
					if(vSpread != null) particle.vSpread = vSpread;
					
					DFVar.setVar(var, new DFValue(particle, DFType.PART), localStorage);
				}
				
				case "GetParticleSize" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = args.get("particle") == null ? (DFParticle) DFVar.getVar(var, localStorage).getVal() : (DFParticle) args.get("particle").getVal();
					
					DFVar.setVar(var, new DFValue(particle.size, DFType.NUM), localStorage);
				}
				...
				case "SetParticleSize" -> {
					...
					particle.size = (double) args.get("size").getVal();
					Double variation = (Double) args.get("variation").getVal();
					if(variation != null) particle.motionVariation = variation;
				...
				case "GetParticleMat" -> {
					...
					DFVar.setVar(var, new DFValue(particle.material.toString().toLowerCase(), DFType.TXT), localStorage);
				}
				...
				case "SetParticleMat" -> {
					...
					particle.material = ((ItemStack) args.get("material").getVal()).getType();
					
					DFVar.setVar(var, new DFValue(particle, DFType.PART), localStorage);
				}
				
				case "GetParticleColor" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = (DFParticle) args.get("particle").getVal();
					Color color = particle.color;
					String hexCode = Integer.toHexString(color.getRed()).toUpperCase() + Integer.toHexString(color.getGreen()).toUpperCase() + Integer.toHexString(color.getBlue()).toUpperCase();
					
					DFVar.setVar(var, new DFValue(hexCode, DFType.TXT), localStorage);
				}
				...
				case "SetParticleColor" -> {
					...
					particle.color = DFUtilities.hex2Rgb((String) args.get("color").getVal());
					Double variation = (Double) args.get("variation").getVal();
					if(variation != null) particle.colorVariation = variation;
					
					DFVar.setVar(var, new DFValue(particle, DFType.PART), localStorage);
				}
				
				case "GetParticleMotion" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = (DFParticle) args.get("particle").getVal();
					
					DFVar.setVar(var, new DFValue(particle.motion, DFType.VEC), localStorage);
				}
				
				case "SetParticleMotion" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					DFParticle particle = args.get("particle") == null ? (DFParticle) DFVar.getVar(var, localStorage).getVal() : (DFParticle) args.get("particle").getVal();
					
					particle.motion = (Vector) args.get("motion").getVal();
					Double variation = (Double) args.get("variation").getVal();
					if(variation != null) particle.colorVariation = variation;
				...
				case "GetBlockType" -> {
					DFVar var = (DFVar) args.get("var").getVal();
					Location loc = (Location) args.get("loc").getVal();
					Material type = DFPlugin.world.getBlockAt(loc).getType();
					
					DFVar.setVar(var, switch(tags.get("Return Value Type")){
						case "Block ID (oak_log)" -> new DFValue(type.toString().toLowerCase(), DFType.TXT);
						case "Block name (Oak Log)" -> new DFValue(new ItemStack(type).getItemMeta().getDisplayName(), DFType.TXT);
						case "Item" -> new DFValue(new ItemStack(type), DFType.ITEM);
						default -> throw new NotImplementedException("Error while processing GetBlockType action: Unknown tag value " + tags.get("Return Value Type"));
					}, localStorage);
				}
			}
		}
