...
public class PlayerAction {
	public static void invokeAction(Object[] inputArray, String action, LivingEntity target){
		...
		switch(action){
				...
				case "SendMessageSeq":{
					sendMessageSeq((Player) target, DFValue.castTxt((DFValue[]) args.get("msgs").getVal()), (long) args.get("delay").getInt(), DFPlugin.plugin);
				...
				case "PlaySoundSeq":{
					...
					playSoundSeq((Player) target, DFValue.castSound((DFValue[]) args.get("sounds").getVal()), loc, (long) args.get("delay").getInt(), DFPlugin.plugin);
					return; // This method already affects all targets, no need to continue main loop
				}
