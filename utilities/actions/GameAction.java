				...
				switch (specialProj.getType()) {
					case FIRE_CHARGE: {
						if (specialProj.getAmount() == 2) {
							...
							if (customName != null) projectile2.setCustomName(customName);
					...
					case TIPPED_ARROW: {
						...
						Arrow projectile2 = (Arrow) projectile.getWorld().spawnEntity(projectile.getLocation(),type);
						PotionMeta meta = (PotionMeta) specialProj.getItemMeta();
						target.sendMessage(meta.getBasePotionData().toString());
						projectile2.setBasePotionData(meta.getBasePotionData());
						if (customName != null) projectile2.setCustomName(customName);
