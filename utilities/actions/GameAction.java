...
public class GameAction {
	public static void invokeAction(Object[] inputArray, String action, LivingEntity[] targets) {
		...
		LivingEntity target = targets[0]; // Game Action is not ran once per selection.
		switch (action) {
			case "SpawnMob": {
			...
			case "LaunchProj": {
				...
				Location loc = (Location) args.get("loc").getVal();
				String customName = (String) args.get("customName").getVal();
				float speed = (float) (double) args.get("speed").getVal();
				float inaccuracy = (float) (double) args.get("inaccuracy").getVal();
				
//					net.minecraft.world.entity.projectile.Projectile projectile = (net.minecraft.world.entity.projectile.Projectile) target.getWorld().spawnArr(loc, loc.getDirection(), speed, inaccuracy);
//					((CraftSnowball) projectile).getHandle().setItem()
				break;
			}
