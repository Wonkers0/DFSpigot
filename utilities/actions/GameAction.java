		...
		Bukkit.getScheduler().callSyncMethod(DFPlugin.plugin, () -> {
			switch (action) {
				case "SpawnMob": {
					...
					Bukkit.broadcastMessage(String.join(", ", Arrays.stream(equipment).map(item -> item == null ? "null" : item.getItemMeta().getDisplayName()).toArray(String[]::new)));
				...
				case "FallingBlock": {
					Location loc = (Location) args.get("loc").getVal();
					Bukkit.broadcastMessage(DFUtilities.parseTxt(args.get("block")));
				...
				case "SpawnArmorStand": {
					...
					if(tags.get("Visibility") == "Visible (No hitbox)") entity.setMarker(true);
					if(tags.get("Visibility") == "Invisible") entity.setVisible(false);
					if(tags.get("Visibility") == "Invisible (No hitbox)"){
