	...
	public void invokeAction() {
		...
		switch (action) {
				...
				case "CloneRegion" : {
					...
					//TODO block contents copying is being ass
			...
			case  "SetBlockData": {
				...
				String[] blockTags = DFValue.castTxt((DFValue[]) args.get("tags").getVal());
				if(tags.get("Overwrite Existing Data") == "True") {
					//TODO this feature is quite redundant and i dont quite know how it works, "if the block has other tags that you don’t modify with the action they either get removed or not" doesnt make sense and it doesnt change ingame if i try 
				}
				for(Location loc : locs){
					if(args.get("tags").getVal() != null){
						BlockData finalData = loc.getBlock().getType().createBlockData();
						StringBuilder builder = new StringBuilder();
						builder.append("[");
						for (String dblockData : blockTags)
							builder.append(dblockData).append(",");
						...
						builder.delete(builder.length() - 1, builder.length());
						builder.append("]");
						BlockData data = loc.getBlock().getType().createBlockData(String.valueOf(builder));
						finalData = data.merge(finalData);
						loc.getBlock().setBlockData(finalData);
					}
				}
