		...
		switch(action){
			...
			case "EventBlockEquals": {
				for(ItemStack itm : DFValue.castItem((DFValue[]) args.get("blocks").getVal()))
					if(DFUtilities.interpretItem(itm.getType()) == ((Block) specifics.get("block")).getType()) return true;
				...
				for(ItemStack itm : DFValue.castItem((DFValue[]) args.get("items").getVal()))
					switch(tags.get("Comparison Mode")){
						case "Exactly equals":
							...
							break;
						case "Ignore stack size/durability": {
							if(checkItem instanceof Damageable) ((Damageable) checkItem).setDamage(((Damageable) itm).getDamage());
