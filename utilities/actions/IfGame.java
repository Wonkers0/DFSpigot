...
public class IfGame {
	public static boolean invokeAction(Object[] inputArray, String action, LivingEntity target, HashMap<String, Object> specifics) {
		...
		switch(action){
			case "BlockEquals": {
				Block block = target.getWorld().getBlockAt((Location) args.get("loc").getVal());
			...
			case "BlockPowered": {
				...
				for(Location loc : locs){
					Block block = target.getWorld().getBlockAt(loc);
					if(block.isBlockPowered() && tag == "Direct power") return true;
					if(block.isBlockIndirectlyPowered() && tag == "Indirect power") return true;
				}
			}
			
			case "ContainerHas": { // TODO: Test this action, I'm quite uncertain whether it'll work or not
				Block container = target.getWorld().getBlockAt((Location) args.get("loc").getVal());
			...
			case "EventBlockEquals": {
				for(ItemStack itm : DFValue.castItem((DFValue[]) args.get("blocks").getVal()))
					if(itm.getType() == ((Block) specifics.get("block")).getType()) return true;
